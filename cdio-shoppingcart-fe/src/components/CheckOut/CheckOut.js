    ...
    MySwal.fire({
      ...
      if (result.value) {
        ...
        cart.map(async item => {
          ...
          const changeNumberAvailableProduct = callApi(`products/${item.id}/updateNumberAvailable`, 'PUT', payloadNumberAvailable, token);
          const addOrderDetail = callApi("orderDetails", "POST", resultOrderDetail, token);
